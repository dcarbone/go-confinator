...
func TestStringDuration(t *testing.T) {
	type (
		...
		testRun struct {
			name string
			fn   func(*testing.T, *confinator.StringDuration, testValue) error
		}
	)
	...
	var testDefinitions = []testRun{
		{
			name: "flag",
			fn: func(t *testing.T, sdv *confinator.StringDuration, tv testValue) error {
			...
			for _, testDefinition := range testDefinitions {
				{
					...
					t.Run(testDefinition.name, func(t *testing.T) {
						t.Parallel()
						var sdv confinator.StringDuration
